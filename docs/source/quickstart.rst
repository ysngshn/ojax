...
:meth:`ojax.OTree.infer_field_type` method and can be overridden by subclasses.
...
This automatic generation is convenient for basic cases but has its limits for
complex cases such as additional checks or some inheritance cases\ [1]_. In
these cases, one should instead implement a custom ``__init__`` method.
...
numerical data for the children fields.
...
don't alter the PyTree structure either. It is also a requirement for JAX
functions such as :obj:`jax.lax.scan`.
